...
def load_data():
    ...
    # Clean data
    df['SUM_ASSURED_CLEAN'] = df['SUM_ASSURED'].str.replace(',', '').astype(float)
    df['PI_ANNUAL_INCOME_CLEAN'] = df['PI_ANNUAL_INCOME'].str.replace(',', '').replace('', '0').replace('0', np.nan)
...
# ============================================================
    ...
    st.dataframe(summary_df.style.highlight_max(subset=['Accuracy', 'Precision', 'Recall', 'F1-Score'], color='lightgreen'), 
...
# ============================================================
    ...
    for idx, (name, res) in enumerate(results.items()):
        with cols[idx % 2]:
            ...
            # Highlight FP and FN
            ax.add_patch(plt.Rectangle((1, 0), 1, 1, fill=False, edgecolor='red', linewidth=3))
            ax.add_patch(plt.Rectangle((0, 1), 1, 1, fill=False, edgecolor='orange', linewidth=3))
            st.pyplot(fig)

            st.markdown(f"""
            - ❌ **False Positives**: {fp} ({fp_pct:.2f}%) - Wrongly Approved (Actual: Repudiated)
            - ❌ **False Negatives**: {fn} ({fn_pct:.2f}%) - Wrongly Repudiated (Actual: Approved)
...
# ============================================================
    ...
    <h3>⚠️ FINDING 3: Age-Related Patterns</h3>
    <ul>
        <li>Policyholders <b>under 40</b> show <b>33.1% repudiation</b> (slightly above average)</li>
        <li>Policyholders <b>80+</b> show <b>0% repudiation</b> (only 1 record - insufficient data)</li>
    ...
    st.markdown("""
    <div class="finding-box">
    <h3>⚠️ FINDING 4: Gender Patterns</h3>
    <ul>
        ...
        <li>However, young females in certain age brackets show elevated rates</li>
    </ul>
    </div>
    """, unsafe_allow_html=True)
    ...
    <ol>
        <li><b>Audit Low-Income Claims</b>: Review all repudiated claims from Very Low income category for procedural fairness</li>
        <li><b>Standardize Zone Practices</b>: Implement uniform claim assessment guidelines across all zones</li>
        <li><b>Bias Training</b>: Conduct mandatory unconscious bias training for all settlement officers</li>
        <li><b>ML Monitoring</b>: Deploy the Gradient Boosting model (AUC = 0.786) as a bias detection tool</li>
    </ol>
    <h4>Long-term Actions:</h4>
    <ol>
        <li><b>Automated Screening</b>: Use the ML model to flag potentially biased decisions for review</li>
        <li><b>Quarterly Bias Audits</b>: Regular statistical analysis of settlement patterns</li>
        <li><b>Income-Blind Review</b>: Consider income-blind claim assessment for initial evaluation</li>
        <li><b>Zone Rotation</b>: Rotate settlement officers across zones to reduce localized bias</li>
    </ol>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.info("💡 **Recommendation**: Deploy **Gradient Boosting** as the primary bias detection model due to its highest AUC (0.786) and best balance of precision/recall. Use **Random Forest** as a secondary model for high-recall scenarios.")
